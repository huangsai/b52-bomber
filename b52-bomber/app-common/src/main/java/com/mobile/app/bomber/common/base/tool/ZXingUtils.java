...
public class ZXingUtils {
    ...
    /**
     * 生成图片 要转换的地址或字符串,可以是中文
     * 不需要logo最后一个参数传nul
     * @param width
     * @param height
     * @return
     */
    public static Bitmap createImage(Bitmap bitmap, final int width, final int height, Bitmap logoBitmap) {
...
//            }
            ...
            hints.put(EncodeHintType.CHARACTER_SET, "utf-8");
            // 图像数据转换，使用了矩阵转换
            BitMatrix bitMatrix = new QRCodeWriter().encode("null",
                    BarcodeFormat.QR_CODE, width, height, hints);
            int[] pixels = new int[width * height];
            // 下面这里按照二维码的算法，逐个生成二维码的图片，
            // 两个for循环是图片横列扫描的结果
